...
public class VendorRest {
	...
	@GET
	@Path("/acceptOrRejectOrder/{ordId}/{venId}/{status}")
	@Produces(MediaType.APPLICATION_JSON)
	public String acceptOrReject(@PathParam("ordId") int ordId,@PathParam("venId") int venId,@PathParam("status") String status) throws SQLException {
		String result =new VendorDAO().acceptOrReject(ordId,venId,status);
		return result;
	}
