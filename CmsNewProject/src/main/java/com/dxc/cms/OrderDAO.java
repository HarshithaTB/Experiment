...
public class OrderDAO {
	...
	public OrderDetail getOrderbyOrderId(int ordId) throws SQLException {
		Connection con = ConnectionHelper.getConnection();
		PreparedStatement pst;
		String cmd = "";
		/*
		 String cmd = "select Count(*) cnt from Order_detail where ORD_ID=?";
		 PreparedStatement pst = con.prepareStatement(cmd); pst.setInt(1, ordId);
		 ResultSet rs = pst.executeQuery(); rs.next();
		 
		int cnt = rs.getInt("cnt");
		OrderDetail[] result = new OrderDetail[cnt];
		*/
		cmd = "select * from Order_detail where ORDER_ID=?";
		pst = con.prepareStatement(cmd);
		pst.setInt(1, ordId);
		ResultSet rs = pst.executeQuery();
		OrderDetail orderDetail = null;
	...
	public OrderDetail[] vendorOrder(int venId) throws SQLException {
	  ...
	  public String cancelMyOrder(int ordId,int venId) throws SQLException {
			String result="";
			double Price;
